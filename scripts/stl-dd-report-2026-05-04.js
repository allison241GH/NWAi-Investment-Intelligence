/**
 * STL — NWAi DD Investment Report (v2)
 * Prepared: May 4, 2026
...
const data = {
  ...
  s8: {
    ...
    dark: true,
    paras: [
      P([T("Verdict:  ", { bold: true, color: "FFFFFF" }), T("Irreplaceable founder; thin bench; named bandwidth bottleneck. Three independent sources — Golden Seeds, NWAi Apr 29, Petro May 1 — flag the same operating constraint: Aditi's 'entrepreneurial trap' (Petro's exact words) gates channel velocity by requiring founder-touch on every customer.", { color: "FFFFFF" })]),
      P([T("Roster:  ", { bold: true, color: "FFFFFF" }), T("Aditi Majumdar (CEO) — UCI CS Distinguished Prof, NSF CAREER, 25+ years multi-projector calibration, Disney Imagineering advisor; maintains UCI professorship; gates every Panasonic customer. CP (CTO) — strong technical depth confirmed in live Mar 2 demo Q&A; maintains UCI role; junior-heavy team underneath. David Snyder (VP Sales) — hired Feb 2026, $140K + 10% commission; HubSpot operational; trial-kit innovation shipped; cannot scale alone. Stephen Clark (Air Force advisor) — previously CEO; 6.6% FD common, fully vested; stepped back to paid consulting; defense-conversion record unverified per Golden Seeds. 4-person engineering team (UCI-trained, 3–8 years tenure); equity refresh planned at seed, not bridge — retention risk if seed slips.", { color: "FFFFFF" })]),
      P([T("Petro's unprompted operating memo (May 1, minute 53):  ", { bold: true, color: "FFFFFF" }), T("Asked 'if you were Aditi, what would you do?' — (1) hire 2–3 senior salespeople with industry relationships; (2) give one influential customer the product free for testimonial, start the flywheel; (3) build the team, give them autonomy, exit the entrepreneurial trap, offer equity to '2–3 high-performing committed generals'; (4) then turn on the engine — healthcare, beyond museums. Read this as the closest thing STL has to a written outside-advisor memo and the basis for NWAi's soft IC term.", { color: "FFFFFF" })]),
      P([T("NWAi soft term recommendation:  ", { bold: true, color: "FFFFFF" }), T("Written hiring + delegation plan tied to seed close. No-cost discipline. Directly addresses the structural contradiction surfaced Apr 29 (NO new GTM hires until seed) versus the funnel-people-constraint reality.", { color: "FFFFFF" })]),
    ],
  },
  ...
  s12: {
    ...
    dark: true,
    paras: [
      P([T("Exit verdict:  ", { bold: true, color: "FFFFFF" }), T("Strategic acquisition is the realistic exit path. Three credible acquirer categories with named candidates. Base case sits at the low end of the 10x threshold; Bull case clears it comfortably. IPO is not the underwriting path.", { color: "FFFFFF" })]),
      P([T("Strategic acquirer landscape:  ", { bold: true, color: "FFFFFF" }), T("Panasonic — operationally embedded; NA CEO sponsorship; three uncodified long-term paths (global, white-label, Japan co-development). Highest-probability acquirer. Christie / Sony / Epson — projector incumbents; trial activity already in flight (TechMD Disney trial, Sony seed interest, Epson golf-simulator pilot). Disguise / Pixera — media-server bundlers; capability gap is exactly STL's product; absorb-the-threat dynamic. Defense primes — discount in base case (SBIR frozen, Clark conversion unverified) but available as upside if defense unlock materializes.", { color: "FFFFFF" })]),
      P([T("10x mechanics (see S9 for entry math):  ", { bold: true, color: "FFFFFF" }), T("Base case borderline — depends on entry valuation at note conversion (cap and discount apply) and 1.5x liquidation preference compounding against later-round investors. Bull case non-trivial probability — three Petro-described long-term paths each independently unlocks bull pricing. Conservative IC posture: credit only the in-flight 80-license test for May 12 IC; treat the three optionalities as upside.", { color: "FFFFFF" })]),
      P([T("Early-exit triggers:  ", { bold: true, color: "FFFFFF" }), T("Strategic acquisition in 2027–2028 (post-seed) is the most plausible early-exit scenario, particularly if Panasonic exercises a path-of-least-resistance buy to lock the technology against Disguise / Pixera bundling. 1.5x early-exit protection is below market but provides downside coverage on early acquisitions.", { color: "FFFFFF" })]),
    ],
  },
  ...
  s13: {
    ...
    dark: false,
    paras: [
      P([T("Consensus view:  ", { bold: true }), T("STL is a small, founder-driven C-Corp with $480K of mostly-non-recurring 2025 revenue, an unproven sales motion, a 6-month Panasonic test that closed ZERO projects in H1 2026, and a $3M seed-round dependency that is existential. A traction investor passes today.")]),
...
// ─── Generate ───────────────────────────────────────────────────────────────
const outFile = path.join(__dirname, "STL-NWAi-DD-Report-2026-05-04.docx");
