...
If a Post-Meeting Layer Manifest is provided, read the latest Action Tracker FIRST — every diligence item with an ✅ RESOLVED status represents a risk that has been mitigated and should weight your valuation upward; every 🔴 OPEN item is a risk that remains and should weight your valuation downward or be flagged as a condition on the deal structure recommendation. **Exception — deal structure & syndication:** a SAFE on the cap table or the absence of an external lead is NOT, on its own, a downward weight — it is a negotiable term and the basis for your structure recommendation. Weight downward only for founder inflexibility on a structure NWA cannot close, or a genuinely dirty/concentrated cap table. Read all post-meeting reports for any analyst observations that change the exit pathway, comp set anchoring, or hurdle rate test. Your final NWA action recommendation (ADVANCE / WATCH / DECLINE) must reflect the most current understanding of the deal, not the Scout-stage understanding.
