...
**Track 1 (Dealflow Intelligence) is live and mature for TechGroup.** The `nwai-tech-pipeline` plugin runs a 7-stage pipeline (Inbox → Screen → Scout → Diligence → DD Report → Decision → Memo) with 8 slash commands, 7 AI research agents (pipeline-monitor, company-researcher, market-analyst, competitive-intelligence, technical-diligence, financial-analyst, risk-assessor), Dealum CRM integration via MCP, GitHub auto-sync, and 9 proprietary framework reference documents encoding NWAi investing criteria. Structured outputs include Triage Reports, Scout Assessments, DD Kickoff Packages, Post-Meeting Reconciliations, DD Investment Reports, and Exec Memos. Two analytical lenses (Structural Discontinuity, Memory Lock-in) are always active. Five non-negotiable investment criteria enforce consistency.
